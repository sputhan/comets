...
Valid Query with date range +D and now
    ...
    ${resp}=    Query With date range    date-min=now    date-max=+100
    ${count_1}    Set Variable    ${resp.json()["count"]}
    ${resp}=    Query With date range    date-min=now    date-max=+30
    ${count_2}=    Set Variable    ${resp.json()["count"]}
    Should Be True    ${count_2}<${count_1}

Valid Query with date range YYYY-MM-DD format
    [Documentation]    Querying within a shorter date range should return smaller count in output
    ${resp}=    Query With date range    date-min=2020-01-01    date-max=2021-01-01
    ${count_1}    Set Variable    ${resp.json()["count"]}
    ${resp}=    Query With date range    date-min=2020-01-01    date-max=2020-05-01
    ${count_2}=    Set Variable    ${resp.json()["count"]}
    Should Be True    ${count_2}<${count_1}

Valid Query with date range YYYY-MM-DDThh:mm:ss format
    [Documentation]    Querying within a shorter date range should return smaller count in output
    ${resp}=    Query With date range    date-min=2020-01-01T00:10:10    date-max=2020-01-01T10:10:10
    ${count_1}    Set Variable    ${resp.json()["count"]}
    ${resp}=    Query With date range    date-min=2020-01-01T05:10:10    date-max=2020-01-01T10:10:10
    ${count_2}=    Set Variable    ${resp.json()["count"]}
    Should Be True    ${count_2}<${count_1}
